...
module Gitsh
  ...
  class TokenZipper
    ...
    # Returns ture if the current token is a short or long option and is not a command.
    #
    # @return [Boolean]
    def option?
    ...
    # Returns true if the option exists for the current command.
    #
    # @return [Boolean]
    def valid_option?
      return false unless current_command

      Git::HelpPage
        .for(command: current_command.token.content)
        .option?(token.raw_content)
    end
  end
end
